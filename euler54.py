...
class HandRank(Enum):
    ...
    STRAIGHT_FLUSH = 9  # All cards are consecutive values of same suit.
    ROYAL_FLUSH = 10  # Ten, Jack, Queen, King, Ace, in same suit.
...
def are_same_value(cards):
    return len(set([card[0] for card in cards])) == 1


def evaluate_hand(hand):
    hand_array = hand.split(" ")
    hand_array = sorted(hand_array, key=lambda card: (VALUES.index(card[0]), SUITS.index(card[1])))

    # Royal flush
    if all([value in hand for value in VALUES[-5:]]):
        if are_same_suit(hand_array):
            return HandRank.ROYAL_FLUSH.value,

    # Straight flush
    if are_same_suit(hand_array):
        if "".join([card[0] for card in hand_array]) in "".join(VALUES):
            return HandRank.STRAIGHT_FLUSH.value, VALUES.index(hand_array[-1][0])

    # Four of a kind
    if are_same_value(hand_array[1:]):
        return HandRank.FOUR_OF_A_KIND.value, VALUES.index(hand_array[1][0]), VALUES.index(hand_array[0][0])
    if are_same_value(hand_array[:-1]):
        return HandRank.FOUR_OF_A_KIND.value, VALUES.index(hand_array[0][0]), VALUES.index(hand_array[-1][0])

    # Full House
    if are_same_value(hand_array[2:]) and are_same_value(hand_array[:2]):
        return HandRank.FULL_HOUSE.value, VALUES.index(hand_array[2][0]), VALUES.index(hand_array[0][0])
    if are_same_value(hand_array[3:]) and are_same_value(hand_array[:3]):
        return HandRank.FULL_HOUSE.value, VALUES.index(hand_array[0][0]), VALUES.index(hand_array[3][0])

    # Flush
    if are_same_suit(hand_array):
        return HandRank.FLUSH.value, VALUES.index(hand_array[4][0]), VALUES.index(hand_array[3][0]), \
            VALUES.index(hand_array[2][0]), VALUES.index(hand_array[1][0]), VALUES.index(hand_array[0][0])

    # Straight
    if "".join([card[0] for card in hand_array]) in "".join(VALUES):
        return HandRank.STRAIGHT.value, VALUES.index(hand_array[-1][0])

    # Three of a kind
    if are_same_value(hand_array[2:]):
        return HandRank.THREE_OF_A_KIND.value, VALUES.index(hand_array[2][0]), VALUES.index(hand_array[1][0]), VALUES.index(hand_array[0][0])
    if are_same_value(hand_array[1:-1]):
        return HandRank.THREE_OF_A_KIND.value, VALUES.index(hand_array[1][0]), VALUES.index(hand_array[-1][0]), VALUES.index(hand_array[0][0])
    if are_same_value(hand_array[:-2]):
        return HandRank.THREE_OF_A_KIND.value, VALUES.index(hand_array[0][0]), VALUES.index(hand_array[-1][0]), VALUES.index(hand_array[-2][0])

    # Two pairs
    if are_same_value(hand_array[:2]):
        if are_same_value(hand_array[2:4]):
            return HandRank.TWO_PAIRS.value, VALUES.index(hand_array[2][0]), VALUES.index(hand_array[0][0]), VALUES.index(hand_array[-1][0])
        if are_same_value(hand_array[-2:]):
            return HandRank.TWO_PAIRS.value, VALUES.index(hand_array[3][0]), VALUES.index(hand_array[0][0]), VALUES.index(hand_array[2][0])
    if are_same_value(hand_array[1:3]):
        if are_same_value(hand_array[3:]):
            return HandRank.TWO_PAIRS.value, VALUES.index(hand_array[-1][0]), VALUES.index(hand_array[2][0]), VALUES.index(hand_array[0][0])

    # One pair
    if are_same_value(hand_array[:2]):
        return HandRank.ONE_PAIR.value, VALUES.index(hand_array[0][0]), VALUES.index(hand_array[-1][0]), \
               VALUES.index(hand_array[-2][0]), VALUES.index(hand_array[-3][0])
    if are_same_value(hand_array[1:3]):
        return HandRank.ONE_PAIR.value, VALUES.index(hand_array[1][0]), VALUES.index(hand_array[-1][0]), \
               VALUES.index(hand_array[-2][0]), VALUES.index(hand_array[0][0])
    if are_same_value(hand_array[2:4]):
        return HandRank.ONE_PAIR.value, VALUES.index(hand_array[2][0]), VALUES.index(hand_array[-1][0]), \
...
def main():
    ...
    count = 0
    for hand in hands:
        player_1_score = evaluate_hand(hand[:14])
        player_2_score = evaluate_hand(hand[14:][1:])
...
hands_text = """8C TS KC 9H 4S 7D 2S 5D 3S AC
